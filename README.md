...
Drag the island (or the capture panel's header) up and down to choose its height. Pull the island inward to stretch its sticky connection to the edge: a short pull settles back, while pulling past the release point catapults it to the opposite wall with a soft landing. You can keep holding the mouse; the island completes the flight on its own. While detached, it has four rounded corners; the bezel joins return when it lands. With reduced motion enabled, the same gesture switches sides immediately.
