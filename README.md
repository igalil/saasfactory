...
Drag the island (or the capture panel's header) up and down to choose its height. Pull the island inward to stretch its sticky connection to the edge, which thins from an hourglass into a thread: a short pull springs back, while pulling past the release point snaps the thread and catapults the island to the opposite wall, where it squashes, settles, and grows its bezel joins back. You can keep holding the mouse; the island completes the flight on its own. While detached, it has four rounded corners. With reduced motion enabled, the same gesture switches sides immediately.

The capture header still drags across the screen normally, and dragging outward onto an adjacent display transfers the island there. The island and capture panel sit flush against the physical left or right screen edge, with outward-curving corners joining the bezel. The island is a slim solid black notch, 32 pixels deep by 80 pixels tall, with an 18-pixel icon, curved shoulders joining the bezel, and no outer outline. Vertical placement stays within the usable screen area and is remembered after restart. With the island focused, **Alt + arrow keys** move it vertically or switch sides. The island icon fades to 60% opacity when unfocused while the black surface stays opaque; the capture panel fades to 80%. Hover and keyboard focus brighten the icon, and keyboard focus also strengthens its stroke. Expansion uses an immediate resize to avoid the old stretching animation.
