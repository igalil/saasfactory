...
const VERDICT_LABELS: Record<string, string> = {
  unproven: 'RESEARCH INCOMPLETE — UNRATED',
...
function getVerdictLabel(verdict: string | undefined): string {
  if (!verdict) return 'RESEARCH INCOMPLETE — UNRATED';
  const key = verdict.toLowerCase() as MarketVerdict;
  return VERDICT_LABELS[key] || 'RESEARCH INCOMPLETE — UNRATED';
}
...
export function displayResearchSummary(research: MarketResearch): void {
  ...
  console.log(`  ${verdictColor(marketValidation.verdict === 'unproven' ? 'Unrated' : `${marketValidation.score}/10`)} ${chalk.dim('[')}${verdictColor(scoreBar)}${chalk.dim(']')} ${verdictColor(verdictLabel)}`);
...
**Market Validation Score:** ${marketValidation.verdict === 'unproven' ? 'Unrated' : `${marketValidation.score}/10`} - ${verdictLabel}
...
` : `
**Validate before committing to a build.** Key actions:
...
export function getResearchOneLiner(research: MarketResearch): string {
  ...
  return `Score: ${research.marketValidation.verdict === 'unproven' ? 'Unrated' : `${score}/10`} (${verdictLabel}) | ${competitorCount} competitors found | ${research.opportunities.length} opportunities identified`;
}
