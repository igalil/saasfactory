...
Native window corner rounding is disabled so it cannot clip the concave bezel joins. The renderer owns the notch silhouette, the capture panel's responsive bezel joins, and the library's rounded corners. During flight, the island uses a symmetric rounded rectangle so the destination-edge state change cannot flip its silhouette before landing.
