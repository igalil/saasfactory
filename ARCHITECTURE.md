...
`main.ts` owns the single-instance lock, tray, global shortcut, and permission handlers. `window-controller.ts` owns the three window modes, native focus state, and edge-constrained dragging; `window-position.ts` aligns the island and capture panel with the physical side edges while keeping vertical placement and the library within the display work area. A separate `window-position.json` saves the display, left/right edge, and normalized island height. Expanded panels retain that anchor, and disconnected displays fall back to an available display without stealing focus. Native resizes are immediate, avoiding layout reflow during macOS window animation. The renderer uses pointer capture for dragging, event-time screen coordinates through validated IPC, and CSS opacity for the unfocused island icon and capture panel. The island's black notch surface and the library remain opaque. Window close collapses; tray Quit flushes placement, stops active work, and exits.

`island-motion.ts` shares sticky-gesture thresholds and timings. Inward island pulls temporarily widen the transparent native window into a horizontal strip. `island-goo.ts` computes each pose (body position and squash, plus the goo joining it to a wall) as pure geometry: a held pull thins an hourglass waist into a thread; a launch snaps it into a wall bead and a retracting tail, then squashes against the far wall while the bezel joins regrow. `IslandSurface.tsx` samples those poses into a compositor transform animation for the island and redraws one SVG goo path from that animation's own timeline; native bounds stay fixed during flight. Passing the threshold commits the destination edge and releases pointer capture, so subsequent mouse movement cannot steer the flight. The strip ignores mouse input during flight/return, and background animation throttling is suspended only for the gesture. A validated animation ID acknowledges completion; a bounded main-process fallback restores narrow, interactive bounds if rendering stalls. Mode/display changes, renderer failure, and shutdown also clean up motion. System reduced-motion settings skip the flight; the renderer additionally checks its media preference. Capture-panel dragging keeps the ordinary edge behavior.

Native window corner rounding is disabled so it cannot clip the concave bezel joins. The renderer owns the notch silhouette, the capture panel's responsive bezel joins, and the library's rounded corners. While in motion, the island body is a symmetric rounded rectangle and the goo layer supplies any bezel joins, so the destination-edge state change cannot flip its silhouette before landing. When an animation ends, the landed goo matches the notch exactly and hands back to the island's own clip-path before the strip narrows.
